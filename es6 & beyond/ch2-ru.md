...
Вам, вероятно, известно, что области видимости в JS всегда определяются через `function`. Если вам необходимо создать блочную область видимости, наиболее распространенный способ это сделать - IIFE (immediately invoked function expression). Например:
...
Однако, теперь мы можем отпределять переменные, связанные с любым блоком, названные(ВНЕЗАПНО) блочная область видимости. Это значит, что все что нам нужно, это пара `{ .. }` для того чтобы создать область видимости. Вместо использования `var`, которая всгда создает переменные, привязанные к функции (или к global), используйте `let`:
...
Создание области видимости через `{ .. }` не идиома JS, но это всегда будет абсолютно валидно. И разработчики на других языках, у которых есть нормальная блочная область видимости, легко распознают этот паттерн.

Я хочу прердложить, что я думаю о наилучшем пути создания блочных переменных с блоком `{ .. }`. Более того, я также советую помещать `let` в начало блока. Если у вас больше одного определения, я бы порекомендовал использовать всего одно ключеное слово `let`.

Стилистически, я всегда предпочитаю помещать `let` на ту же линию, что и `{`, чтобы сделать понятнее, что блок нужен только для определения области видимости для тех переменных.
...
Теперь это выглядит странно и не соответствует рекомендациям другой литературы по ES6. Но у меня свои причины для безумия.

Это другая предложенная форма для блока `let`:
...
Эта форма, которую я называю *явная* область видимости , тогда как `let ..` форма, которая похожа на `var` и более *неявная*. В основном разработчики находят *явную*  форму более предпочтительной.

Если вы сравните две формы, они очень похожи, но по моему мнению оба стилистически определяют блочкую область видимости. К сожалению форма `let (..) { .. }` самая *явная* из всех, но не была принята в ES6. Она может появиться вновь после ES6, но сейчас первая форма - лучшая ставка.

Чтобы усилить *неявную* природу `let ..`, рассмотрим следующие примеры:
...
Какие переменные существую только в блоке `if`, и какие переменные существуют только в внутри цикла `for`?

Ответ: `if` блок содержит `b` и `c` переменные, и цикл `for` содержит `i` и `j`.

Вы задумались на этим на мгновение? Вас удивило, что `i` не была добавления во внешний блок `if`? 

Есть так же опасность с определением `let c = ..`, появляющимся в самом низу блока. В отличии от переменных, определяемых через `var`, которые присоединяются ко всей внешней функции, не смотря на то где появилось ее определение, определение `let` присоединяется к блоку, но не инициализируется до тех пор, пока не появится в блоке.

Доступ к переменным, определенным через `let` до самого `let ..`-определения вызывает ошибки, тогда как в определении `var` порядок не важен (исключая стиль).
...
**Внимание:* Эта ошибка `ReferenceError` из-за доступа к переменной обьявленной через `let` технически называется *TDZ* (temporal dead zone) -- вы пытаетесь вызвать переменную, которая определена, но еще не инициализированна. Это не идинственный случай появления *TDZ* -- она возникает в нескольких местах ES6. Так же, заметтьте что инициализация не требует явного присваивания значения в коде, например `let b;` абсолютно корректно. Переменная, которой не присвоено значение во время определения считается что ей присвоено значение `undefined`, `let b;` это все равно что `let b = undefined;`. Явно ли присвоено значение или нет, вы не имеет доступа к переменной `b` до выражения `let b;`.

One last gotcha: `typeof` behaves differently with *TDZ* variables than it does with undeclared (or declared!) variables.

Последний совет: `typeof` ведет себя по разному с *TDZ*, с Обьявленными и не Обьявленными переменными.
...
`a` не обьявлена, а `typeof` единственный безопасный способ проверить, существует она или нет. Но `typeof b` выбрасывает *TDZ* ошибку так как дальше по коду происходит `let b` объявление.

Теперь должно быть понятнее, почему я рекомендую -- нет, я настаиваю -- все `let` -объявления должны быть в начале области видимости. Это абсолютно исключает случайные ошибки доступа слишком рано. Это так же делает обьявления более *явными*, когда вы смотрите на начало блока, любого блока, какие переменные он содержит.

Ваши блоки не должны делить их начальное поведение с поведением областей видимости.

Явность на вашей ответственности, вы должны придерживаться дисциплины, и это сбережет много времени на рефакторинг.
...
Единственное исключение, которое я бы сделал в продпочтении *явной* формы `let` объявления - это `let` появляющаяся в заголовке цикла `for`. Причина может выглядеть нюансом, но я рассматриваю это как одну из наиболее важных ES6 фич.
...
`let i` в заголовке `for` объявляет переменную `i` не просто для самого цикла `for`, но и переобъявляет новую `i` для каждой итерации цикла. Это значит что замыкания, созданные в теле цикла замыкают ту переменную на каждую итерацию, как и лжидается.

Если вы попытаетесь выполнить тот же отрывок, но с `var i` в заголовке цикла `for`, вы получите `5` вместо `3`, так как будет одна переменная `i` во внещней области видимости, которая будет замкнута, вместо новой `i` для каждой итерации.
...
Вы так же могли выполнить то же самое, но чуть длинее:
...
Здесь мы сами создаем новую переменную `j` для каждой итерации, а затем замыкаем ее тем же способом. Я предпочитаю предыдущий подход; это дополнительная специальная возможность, вот почему я поддерживаю форму `for (let .. ) ..`. Это может быть спорно, это кое-что более *неявное*, но это и достаточно *явно*, и достаточно полезно, для моих вкусов.

### `const` Declarations

Еще одна форма определения блочной области видимости, рассмотрим `const`, которая создает *константы*.

Что такое константы? Это переменные, которые становятся только для чтения, после того как им присвоено начальное значение. Рассмотрим:
...
Вам не разрешено присваивать значение переменной, значение которой уже было однажды присвоено, во время объявления. Объявление `const` должно иметь явную инициализацию. Если вы хотите *константу* со значением `undefined`, вы должны объявить ее `const a = undefined`.

*Константы* не ограничивают само значение, а присваивание переменной другого значения. Другими словами, заморожено не значение, а присваивание. Если значение комплексное, как объект или массив, внутреннее состояние может быть изменено:
...
Переменная `a` на самом деле содержит не *константный* массив, она содержит *константную* ссылку на массив; сам массив можно изменять.

**Warning:** Назначение объекта или массива как константы означает, что значение не может быть удалено сборщиком мусора до тех пор пока код выполняется в области видимости этой константы, ссылка на значение может быть никогда не удалено. Это может быть желательно, но будьте осторожны, если это не ваша цель.

По существу, `const` объявление обеспечивает соблэдение которые мы годами выделяли стилистически, мы объявляли имя переменных в верхнем регистре и присваивали им какое-то литеральное значение которое мы старались никогда не изменять. Объявление через `var` не обеспечивало соблюдение этих правил, но теперь объявление через `const` может помочь нам избежать нежелательных изменений.

Есть некоторые предположения, что `const` будет более оптимизированно для JS-интерпретаторов чем `let` или `var`, если интерпретатор будет знать, что переменная никогда не изменится это может исключить возможное слежение.

### Block-scoped Functions

Начиная с ES6, объявление функций внутри блока теперь видна только внутри этого блока. До ES6 спецификация этого не требовала, но много реализаций делало это. Теперь спецификация отвечат реальности.
...
Функция `foo()` объявлена внутри блока `{ .. }`, и для ES6 она будет видна только внутри блока. Она недоступна снаружи. Но так же заметьте "всплытие" внутри блока, что не соответствует объявлению через `let`.

Объявление функций внутри блоков может быть проблемой, если вы когда-нибудь писали код как этот и полагались на старое наследие без блочной области видимости:
...
До ES6, `foo()` напечатает `"2"` несмотря на значение `something`, оба объявления функций всплыли, и второе объявление всегда побеждает.

Но в ES6, последняя строка выбросит исключение `ReferenceError`.

## Spread / Rest

ES6 вводит новый оператор `...` который обычно называют *spread* или *rest*, в зависимости от того, где и как он используется. Давайте рассмотрим:
...
Когда `...` используется перед массивом (на самом деле, любом *iterable*, которые мы рассмотрив а главе 3), он производит "spread" (распространение) его на индивидуальные значения.
